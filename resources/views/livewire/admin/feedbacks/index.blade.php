<div class="space-y-4 sm:space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-2.5 sm:gap-3 min-w-0">
        <div class="col-span-2 lg:col-span-1 bg-surface border-2 border-ink p-3 sm:p-3.5 shadow-brutal min-w-0 flex items-center justify-between lg:block">
            <div class="min-w-0">
                <span class="text-xs font-display font-black uppercase tracking-wider text-ink/70 block">TOTAL MASUKAN</span>
                <span class="text-[10px] sm:text-xs text-ink/50 mt-0.5 block">Semua Tanggapan</span>
            </div>
            <div class="font-display font-black text-2xl sm:text-3xl text-brand lg:mt-1 shrink-0">
                {{ number_format($stats['total'], 0, ',', '.') }}
            </div>
        </div>

        <div class="bg-surface border-2 border-ink p-2.5 sm:p-3.5 shadow-brutal min-w-0 flex flex-col justify-between">
            <span class="text-[10px] sm:text-xs font-display font-bold uppercase tracking-tight sm:tracking-wider text-amber-800 block leading-tight">RATA-RATA RATING</span>
            <div class="font-display font-black text-lg sm:text-2xl text-amber-600 mt-1 flex items-baseline gap-1">
                <span>{{ number_format($stats['average_rating'], 1, ',', '.') }}</span>
                <span class="text-xs font-sans text-ink/50 font-normal">/ 5.0</span>
            </div>
            <span class="text-[10px] sm:text-xs text-ink/50 mt-0.5 block">Indeks Kepuasan</span>
        </div>

        <div class="bg-surface border-2 border-ink p-2.5 sm:p-3.5 shadow-brutal min-w-0 flex flex-col justify-between">
            <span class="text-[10px] sm:text-xs font-display font-bold uppercase tracking-tight sm:tracking-wider text-blue-800 block leading-tight">DENGAN ULASAN</span>
            <div class="font-display font-black text-lg sm:text-2xl text-blue-700 mt-1">
                {{ number_format($stats['with_comment'], 0, ',', '.') }}
            </div>
            <span class="text-[10px] sm:text-xs text-ink/50 mt-0.5 block">Komentar Tertulis</span>
        </div>

        <div class="bg-surface border-2 border-ink p-2.5 sm:p-3.5 shadow-brutal min-w-0 flex flex-col justify-between">
            <span class="text-[10px] sm:text-xs font-display font-bold uppercase tracking-tight sm:tracking-wider text-emerald-800 block leading-tight">RATING POSITIF</span>
            <div class="font-display font-black text-lg sm:text-2xl text-emerald-700 mt-1">
                {{ number_format($stats['positive'], 0, ',', '.') }}
            </div>
            <span class="text-[10px] sm:text-xs text-ink/50 mt-0.5 block">Bintang 4 & 5</span>
        </div>

        <div class="bg-surface border-2 border-ink p-2.5 sm:p-3.5 shadow-brutal min-w-0 flex flex-col justify-between">
            <span class="text-[10px] sm:text-xs font-display font-bold uppercase tracking-tight sm:tracking-wider text-rose-800 block leading-tight">PERLU EVALUASI</span>
            <div class="font-display font-black text-lg sm:text-2xl text-rose-700 mt-1">
                {{ number_format($stats['critical'], 0, ',', '.') }}
            </div>
            <span class="text-[10px] sm:text-xs text-ink/50 mt-0.5 block">Bintang 1 & 2</span>
        </div>
    </div>
        ...
        <div class="flex items-center justify-between gap-2 sm:gap-3 min-w-0">
            <div class="relative flex-1 min-w-0">
                ...
                <input type="text"
                       ...
                       placeholder="Cari isi masukan, nama, NIM..."
            ...
            <div class="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
                <button wire:click="openFilterModal"
                        ...
                        class="inline-flex items-center justify-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-2 border-2 border-ink shadow-brutal-sm font-display font-bold text-xs uppercase tracking-wider transition-all cursor-pointer min-h-10 {{ $activeFilters > 0 ? 'bg-accent text-ink' : 'bg-surface-muted hover:bg-accent text-ink' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <select wire:model.live="perPage"
                        ...
                        class="bg-surface-muted border-2 border-ink px-2 sm:px-2.5 py-2 text-xs sm:text-sm font-sans font-bold text-ink shadow-brutal-sm focus:outline-none focus:bg-surface cursor-pointer min-h-10">
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto flex items-end sm:items-center justify-center p-2.5 sm:p-4 bg-ink/70 backdrop-blur-xs"
            ...
            <div class="w-full sm:max-w-lg bg-surface border-2 border-ink shadow-brutal max-h-[92dvh] sm:max-h-[90vh] flex flex-col min-w-0">
                ...
                <div class="px-4 py-3 sm:px-5 sm:py-3.5 border-t-2 border-ink flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-2.5 sm:gap-3 bg-surface-muted shrink-0">
                    <button wire:click="resetFilters"
                            type="button"
                            class="w-full sm:w-auto px-4 py-2 bg-surface hover:bg-red-50 text-red-700 border-2 border-ink font-display font-black text-xs uppercase tracking-wider transition-colors cursor-pointer text-center">
                        RESET
                    </button>

                    <button wire:click="closeFilterModal"
                            type="button"
                            class="w-full sm:w-auto px-5 py-2.5 bg-brand text-surface hover:bg-brand-dark border-2 border-ink shadow-brutal-sm font-display font-black text-xs uppercase tracking-wider transition-all cursor-pointer text-center">
                        TERAPKAN FILTER
                    </button>
                </div>
            </div>
    ...
    @if ($showDetailModal && $detail)
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto flex items-end sm:items-center justify-center p-2.5 sm:p-4 bg-ink/70 backdrop-blur-xs"
            ...
            <div class="relative w-full max-w-xl bg-surface border-2 border-ink shadow-brutal max-h-[92dvh] sm:max-h-[90vh] flex flex-col min-w-0">
                <div class="px-4 py-3.5 sm:px-6 border-b-2 border-ink flex items-center justify-between bg-surface-muted shrink-0">
                    <div>
                        <span class="text-[11px] font-display font-bold uppercase tracking-wider text-ink/60 block">INFORMASI LENGKAP</span>
                        <h3 id="detail-modal-title" class="font-display font-black text-base sm:text-lg text-brand uppercase truncate min-w-0">
                ...
                <div class="p-4 sm:p-5 border-t-2 border-ink bg-surface-muted flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-2.5 sm:gap-3 shrink-0">
                    @if ($isSuperAdmin)
                        <button wire:click="openDeleteModal({{ $detail->id }})"
                                type="button"
                                class="w-full sm:w-auto px-3.5 py-2 bg-rose-100 hover:bg-rose-200 border-2 border-ink text-xs font-display font-bold text-rose-900 shadow-brutal-sm uppercase cursor-pointer min-h-10 text-center">
                            HAPUS MASUKAN
                        </button>
                    @else
                        <div></div>
                    ...
                    <button wire:click="closeDetailModal"
                            type="button"
                            class="w-full sm:w-auto px-5 py-2 bg-surface border-2 border-ink text-xs font-display font-bold hover:bg-accent shadow-brutal-sm uppercase cursor-pointer min-h-10 text-center">
                        TUTUP
                    </button>
                </div>
            </div>
    ...
    @if ($showDeleteModal)
        <div class="fixed inset-0 z-50 overflow-y-auto flex items-end sm:items-center justify-center p-2.5 sm:p-4 bg-ink/70 backdrop-blur-xs"
            ...
            <div class="relative w-full max-w-md bg-surface border-2 border-ink shadow-brutal max-h-[92dvh] sm:max-h-[90vh] flex flex-col min-w-0">
                <div class="px-4 py-3.5 sm:px-6 border-b-2 border-ink bg-rose-100 flex items-center justify-between shrink-0">
                    <div class="flex items-center gap-2">
                        <span class="w-3 h-3 bg-rose-600 inline-block border border-ink"></span>
                        <h3 id="delete-modal-title" class="font-display font-black text-base text-rose-950 uppercase truncate min-w-0">
                ...
                <div class="p-4 sm:p-6 space-y-3 font-sans overflow-y-auto flex-1 min-w-0">
                ...
                <div class="p-4 sm:p-5 border-t-2 border-ink bg-surface-muted flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2.5 sm:gap-3 shrink-0">
                    <button wire:click="closeDeleteModal"
                            type="button"
                            class="w-full sm:w-auto px-4 py-2 bg-surface border-2 border-ink text-xs font-display font-bold hover:bg-accent shadow-brutal-sm uppercase cursor-pointer min-h-10 text-center">
                        BATAL
                    </button>
                    <button wire:click="deleteFeedback"
                            type="button"
                            class="w-full sm:w-auto px-5 py-2 bg-red-600 text-white border-2 border-ink text-xs font-display font-bold hover:bg-red-700 shadow-brutal-sm uppercase cursor-pointer min-h-10 text-center">
                        YA, HAPUS
                    </button>
                </div>
            </div>
